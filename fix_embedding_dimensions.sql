...
-- 3. RAGドキュメントテーブル作成（1536次元に変更）
-- text-embedding-3-smallまたはtext-embedding-adaを使用
CREATE TABLE rag_documents (
  ...
  -- OpenAI埋め込み（1536次元に変更 - text-embedding-3-small対応）
  embedding vector(1536),              -- メイン埋め込み
  content_embedding vector(1536),      -- コンテンツ埋め込み
  title_embedding vector(1536),        -- タイトル埋め込み
  
  -- 将来的に3072次元対応時のためのテキスト保存
  ...
  embedding_model text DEFAULT 'text-embedding-3-small',  -- モデル変更
...
-- 14. 埋め込みモデル情報表示
SELECT 
  '埋め込みモデル設定' as info_type,
  'text-embedding-3-small (1536次元)' as current_model,
  'pgvector 2000次元制限対応済み' as compatibility,
  'text-embedding-ada-002も利用可能' as alternative;

-- 15. インデックス確認
SELECT 
  indexname,
  CASE 
    WHEN indexdef LIKE '%hnsw%' THEN 'HNSW (1536次元対応)'
...
-- 完了メッセージ
SELECT '🎉 RAGシステム構築完了！text-embedding-3-small（1536次元）対応、pgvector次元制限クリア済み。' as final_status;